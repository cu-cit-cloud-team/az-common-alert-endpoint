...
export const getPeerStatus = async ({
  ...
  const effectivePeersData = peersData || (await getPeersStatus());
  return effectivePeersData[peer];
};
...
export const setPeerStatus = async ({ peer, status, peersData = null }) => {
  const effectivePeersData = peersData || JSON.parse(await getPeersStatus());
  effectivePeersData[peer] = Boolean(status);
  await setPeersStatus(effectivePeersData);
...
export const initPeersStatus = async () => {
  try {
    ...
    console.log(
      'Failed to retrieve peers status; initializing with default content.',
      error
    );
