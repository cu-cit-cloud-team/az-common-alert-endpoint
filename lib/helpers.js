...
const { NOTIFICATION_TIMEZONE = 'America/New_York' } = process.env;
...
export const localizeDateTime = ({
  ...
  (date instanceof Date || typeof date === 'string' ? dayjs(date) : dayjs())
    .tz(tz)
    .format(dateFormat);
...
export const getColorBasedOnSev = (sevString) => {
  ...
  return severityColors[sevString] || '';
};
...
export const getSevDescription = (sevString) => {
  ...
  return severityDescriptions[sevString] || '';
};
